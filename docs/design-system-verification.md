...
The precision audit covers quantity, piece/total dough weight, both ingredient modes, flour blends, all percentages, starter/levain/seed fields, and the ratio readout. Field-specific fraction limits replace the generic 12-significant-digit display. Gram previews share the input precision rules; batch summaries use whole grams and yeast summaries retain three decimals. Typed buffers and raw calculations remain separate, and tiny nonzero values do not become zero. Added formatter regressions and production stories cover the reported 908.8242755752875g example, focus/blur without mutation, fractional editing, small quantities, and rounded workbench fields. Standalone tests and browser checks remain paused at the user’s request; these new assertions are not reported as executed.
