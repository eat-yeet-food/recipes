...
# Production preparation — 2026-09-13

The owner explicitly directed production deployment after disclosure of staging performance shortfalls, missing production credentials and pending owner acceptance, and authorized the required access changes. Cloudflare's `Eat Yeet staging` token was updated: Access Organizations, Access Identity Providers and Cloudflare Pages changed from Read to Edit; the existing account and `eatyeet.com` restrictions remain. The operator setup generated separate production secrets and preserved existing bootstrap/staging secrets. Its combined recovery export is pending the owner's separately entered passphrase. No production resources or cutover route have been created in this preparation step.

Fresh staging three-run mobile medians on the deployed `3f3da9b25244` application: home 88 / LCP 3454 ms; recipes 95 / 2626 ms; browse 77 / 4544 ms; learn 96 / 2478 ms; pizza 97 / 2334 ms; mixing-dough article 94 / 2956 ms. All CLS medians are below 0.1. Four routes miss the 2500 ms LCP target. The report is retained under ignored `dist/remote-performance.json`; a Browse diagnostic additionally reports early image discovery/high priority, so these failures must not be attributed to lazy loading without further investigation. Staging has zero Payload owners; the user's successful Access/page sign-in does not attest Payload admin/session review.

The owner-directed exception path records actual metrics, `performanceVerified: false` and/or `ownerReviewed: false`, the stated authorization, exact commit and a 24-hour expiry. It cannot waive Worker/security checks, tested restore or locking. Production copies acceptance into its release journal. The normal path remains passing acceptance; future releases do not inherit this request's exception authority.

Added `docs/cloudflare-operations.md` and the `cloudflare-operations` repository skill, linked from the canonical guide. Verification: 43 remote tests pass, infrastructure TypeScript, project TypeScript, boundaries and skill validation pass. Application rendering and dependencies are unchanged by these CLI/documentation edits. Live staging acceptance of the final cutover commit, restoration rehearsal, production provisioning/deployment and production verification remain to be executed after recovery enrollment.
