 ...
 * load — used to look like three different products. TanStack's stock error
 * component renders an unstyled "Something went wrong! / Hide Error" panel
 * outside the site's type and color entirely, which is what a visitor saw when
 * the edge served HTML for a JS module.
 *
...
function DeadEnd({
  ...
  return (
    <div className="mx-auto max-w-[var(--max-width)] px-8 py-20 text-center max-sm:px-4">
      <h1 className="font-display text-4xl font-extrabold text-charcoal max-sm:text-3xl">
        {title}
      </h1>
      <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-charcoal/65">{message}</p>
      <div className="mt-10 flex flex-wrap items-center justify-center gap-4">{children}</div>
    </div>
  )
}

/** The filled pink control, matching the hero's "Let's Eat". */
const PRIMARY =
  'inline-flex items-center gap-2.5 rounded-full bg-pink px-8 py-4 text-sm font-semibold uppercase tracking-[2px] text-white shadow-[0_4px_20px_rgba(233,30,99,0.2)] transition-all hover:-translate-y-0.5 hover:bg-pink-dark hover:shadow-[0_8px_30px_rgba(233,30,99,0.3)]'

/** The quiet text control, matching "View all categories". */
const SECONDARY =
  'inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[2px] text-pink transition-colors hover:text-pink-dark'
...
export function NotFound() {
  return (
    ...
    >
      <Link to="/recipes" className={PRIMARY}>
        All Recipes
