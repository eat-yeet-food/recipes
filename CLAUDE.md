...
Standard tooling and routine operations default to the full Git-based release workflow. For explicitly requested code-only iterations, `pnpm run deploy --env staging|production --code-only` verifies a healthy prior release, identical migrations and a fresh no-op content/media plan under the shared lock. It verifies existing media bytes via stored digest/length/MIME, then skips media uploads and migration/sync work. It retains backups, immutable build assets, a brief drain/verification window, exact-commit production acceptance and normal recovery. Use a full release for content/schema changes or missing media; never apply untracked hot patches to an environment.
