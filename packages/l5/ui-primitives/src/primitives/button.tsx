...
const buttonVariants = cva(
  // rounded-full, not shadcn's stock rounded-lg: this app's buttons (the
  // icon-only nav/menu buttons wired here, and every hand-flattened variant
  // before them) are pill-shaped. One-word edit so every future <Button>
  // inherits the site's actual shape by default instead of overriding it
  // per call site (§5.1).
  "group/button inline-flex shrink-0 items-center justify-center rounded-full border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-danger aria-invalid:ring-3 aria-invalid:ring-danger/20 dark:aria-invalid:border-danger/50 dark:aria-invalid:ring-danger/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        // hover:bg-accent (not shadcn's stock hover:bg-muted): this app's
        // accent token is a warm gold tint, not a neutral gray, and the
        // hand-flattened GhostButton it replaces already used it -- gray
        // would be a real visible regression on the nav's icon buttons,
        // not just different wording for the same colour (§5.1.4).
        ghost:
