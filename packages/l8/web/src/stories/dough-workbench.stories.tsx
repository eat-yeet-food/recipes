...
export const SourdoughProcessAndSavedStarter: Story = { args: { target: true }, play: async ({ canvasElement }) => {
  ...
  await expect(screen.getByRole('textbox', { name: 'Autolyse before bulk hours' })).toHaveValue('3')
  await expect(screen.getByRole('textbox', { name: 'Autolyse before bulk minutes' })).toHaveValue('0')
  await expect(screen.getByRole('textbox', { name: 'Planned end of bulk hours' })).toHaveValue('4')
  await expect(screen.getByRole('textbox', { name: 'Planned end of bulk minutes' })).toHaveValue('50')
  await expect(screen.getByRole('textbox', { name: 'Step 2 at hours' })).toHaveValue('1')
  const fold = screen.getByRole('textbox', { name: 'Step 2 at minutes' })
  await userEvent.clear(fold)
  await userEvent.type(fold, '60')
  await userEvent.tab()
  await expect(fold).toHaveAttribute('aria-invalid', 'true')
  await expect(screen.getByRole('button', { name: 'Apply to recipe' })).toBeDisabled()
  await userEvent.clear(fold)
  await userEvent.type(fold, '0')
  await userEvent.tab()
  await expect(screen.getByRole('button', { name: 'Apply to recipe' })).toBeDisabled()
  await userEvent.clear(fold)
  await userEvent.type(fold, '35')
  await userEvent.tab()
  await expect(screen.getByRole('button', { name: 'Apply to recipe' })).toBeEnabled()
  ...
  await expect(screen.getByRole('textbox', { name: 'Step 2 at minutes' })).toHaveValue('17')
  await expect(screen.getByRole('textbox', { name: 'Step 3 at minutes' })).toHaveValue('35')
...
export const PluginProcessProjection: Story = { play: async () => {
  ...
  await expect(instructions).toContain('[1:35h elapsed]')
  await expect(instructions).toContain('[5:20h elapsed]')
  await expect(instructions).toContain('rest for 3h')
  await expect(instructions).toContain('[0m elapsed]')
  await expect(instructions).toContain('[1h elapsed]')
