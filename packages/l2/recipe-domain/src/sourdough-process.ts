export interface SourdoughProcess {
  ...
  bulkMinutes: number
  folds: Array<{ id: string; atMinutes: number }>
}
...
export function sourdoughProcessErrors(process: SourdoughProcess): string[] {
  ...
  if (process.saltDelayMinutes >= process.bulkMinutes) errors.push('Add salt before the end of bulk fermentation.')
  if (new Set(process.folds.map((fold) => fold.id)).size !== process.folds.length) errors.push('Each fold needs a unique identity.')
  ...
  for (const fold of process.folds) {
    if (fold.atMinutes <= previous || fold.atMinutes >= process.bulkMinutes) {
      errors.push('Schedule folds in increasing order after adding salt and before bulk fermentation ends.')
      break
    }
...
export function sourdoughTimeline(process: SourdoughProcess) {
  return [
    ...
    ...process.folds.map((fold) => ({ ...fold, kind: 'fold' as const })),
