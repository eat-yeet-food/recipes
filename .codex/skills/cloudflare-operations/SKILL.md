...
Start by inspecting the clean Git SHA, intended environment, current release status and credential/recovery readiness. Treat user confirmation of a working page as page verification, not an unperformed owner/session/restore attestation. Preserve the user's authorization across turns; do not repeat permission questions for the same approved operation.
